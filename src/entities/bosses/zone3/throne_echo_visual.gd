...
## 왕좌의 잔영(Throne Echo) 보스 잠정 비주얼 — 단순 ColorRect.
## 정식 프로그래밍 아트는 추후 art-spec 작업 시 mire_mother_visual.gd 패턴 답습으로 작성.
## 페이즈 변화는 색조 변경으로 표시 (3페이즈 = 3색).

const BODY_WIDTH: float = 56.0
const BODY_HEIGHT: float = 72.0

## P1: 폐허 갈색 (기본)
const COLOR_PHASE_0 := Color(0.32, 0.27, 0.22, 1.0)
## P2: 그림자 왕가 잔재 보라 (시안 잔영 톤)
const COLOR_PHASE_1 := Color(0.45, 0.35, 0.55, 1.0)
## P3: 가속 황금 잔영
const COLOR_PHASE_2 := Color(0.65, 0.45, 0.30, 1.0)
