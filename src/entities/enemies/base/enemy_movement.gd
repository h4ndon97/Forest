...
func calculate_velocity(
	...
	global_pos: Vector2, _is_on_floor: bool, delta: float
) -> Vector2:
...
func _calculate_airborne(
	state: int, target: Node2D, current_velocity: Vector2,
	global_pos: Vector2, delta: float
) -> Vector2:
	var vel := current_velocity
	...
	match state:
		EnemyStateMachine.State.DORMANT, EnemyStateMachine.State.IDLE, \
		EnemyStateMachine.State.PATROL, EnemyStateMachine.State.HURT, \
		EnemyStateMachine.State.DEAD:
			vel = vel.lerp(Vector2.ZERO, lerp_factor)

		EnemyStateMachine.State.CHASE:
			if is_instance_valid(target):
				var desired: Vector2 = (target.global_position - global_pos).normalized() * max_speed
