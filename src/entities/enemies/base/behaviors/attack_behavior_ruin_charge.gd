...
## 폐허 옛 보초 광역 충격파 공격 (zone3 old_sentinel 적용).
## telegraph 후 짧고 둥근 광역 hitbox 활성. ground_tether 패턴 답습 + 광역 형상 + 폐허 톤.
## telegraph 길이: stats_data.projectile_telegraph 재사용.
## 의도: 옛 보초가 시야 내 플레이어 발견 시 부서진 갑주가 충격파 발산.
## 시각 단서: telegraph 동안 옅은 갈색, active 동안 강한 주황 ColorRect.

const _DEFAULT_ACTIVE_DURATION := 0.35
const FALLBACK_TELEGRAPH_COLOR := Color(0.55, 0.40, 0.25, 0.4)
const FALLBACK_ACTIVE_COLOR := Color(0.95, 0.55, 0.20, 0.7)
...
func _strike() -> void:
	...
	spec.tags = PackedStringArray(["enemy_melee", "ruin_charge"])
