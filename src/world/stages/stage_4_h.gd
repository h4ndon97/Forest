...
## 4-H '그림자 결정의 자리' — 숨김(HiddenRevealer로 진입), 6시 새벽.
## 적 2 (echo_vanguard × 2 — 정지 면역 엘리트가 결정 영역 위협).
## **추가조작: ShadowCrystal 1개** — 동조 시 `world.zone4.shadow_crystal_attuned` flag set.
## zone5 *최종 보스실 진입 조건* (zone5 본 진행은 미동조자도 가능).

const STAGE_ID := "stage_4_h"
const SPAWN_POSITION := Vector2(80, 320)
const BG_PATH := "res://assets/backgrounds/bg_stage_4_h.png"
