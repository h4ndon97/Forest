...
## 네 번째 테스트 스테이지. 황혼(18시), PURIFY 잠금, 적 5.
## 좁은 복도 + 단차 구조로 밀집 전투 테스트.

const BG_PATH := "res://assets/backgrounds/bg_test_stage_4.png"
...
func _ready() -> void:
	...
	EventBus.stage_entered.emit("test_stage_4")
	EventBus.spawn_point_set.emit(Vector2(48, 320))
	StageCamera.apply("test_stage_4")
...
func _try_apply_platforms() -> void:
	...
	if not tex:
		return
	for platform_name in ["PlatformLow", "PlatformMid"]:
