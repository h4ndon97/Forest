...
## 1-5 '부서진 사당' — 정화 도입 + 1-6 진입 잠금 센서 배치.
## 22시 밤, PURIFY 잠금(stage_1_2 완전클리어 필요), 차폐물 2 + 반사 바닥 1, 적 4.
## 숨김: 잔류 정화 → 사당 비문 노출 (PURIFICATION).
## 1-6 잠금 해제: 반사 바닥 위 센서 — 등불 ON 시 반사광으로 점등.

const STAGE_ID := "stage_1_5"
const SPAWN_POSITION := Vector2(80, 320)
const BG_PATH := "res://assets/backgrounds/bg_stage_1_5.png"
