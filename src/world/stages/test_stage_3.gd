...
## 세 번째 테스트 스테이지. 새벽(06시), 잠금 없음, 적 3.
## 수직 구조 레이아웃으로 플랫폼 탐색 테스트.

const BG_PATH := "res://assets/backgrounds/bg_test_stage_3.png"
...
func _ready() -> void:
	...
	EventBus.stage_entered.emit("test_stage_3")
	EventBus.spawn_point_set.emit(Vector2(320, 320))
	StageCamera.apply("test_stage_3")
...
func _try_apply_platforms() -> void:
	...
	if not tex:
		return
	for platform_name in ["Platform1", "Platform2", "Platform3", "Platform4"]:
