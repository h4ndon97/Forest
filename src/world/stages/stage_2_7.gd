...
## 2-7 '마지막 늪' — LIGHT 잠금, 보스 직전 종합 검증.
## 23시(밤), 적 5 — zone2 5종 모두 (weeping_willow + mire_stone + miasma_lily +
## drowned_pillar + marsh_tendril). 짙은 안개 + 등불 강제. 클리어 후 보스(stage_2_b) 진행.

const STAGE_ID := "stage_2_7"
const SPAWN_POSITION := Vector2(80, 320)
const BG_PATH := "res://assets/backgrounds/bg_stage_2_7.png"
