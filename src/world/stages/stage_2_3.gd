...
## 2-3 '가라앉은 길' — 짙은 안개, LIGHT 잠금(등불 ON 강제 진입).
## 22시(밤), 적 4 (weeping_willow + drowned_pillar + marsh_tendril + mire_stone).
## marsh_tendril 첫 도입 — 지면 고정 + telegraph 후 긴 hitbox 학습.
## 환경: FogOverlay 더 진하게 (학습 끝, 위협으로 작용).

const STAGE_ID := "stage_2_3"
const SPAWN_POSITION := Vector2(80, 320)
const BG_PATH := "res://assets/backgrounds/bg_stage_2_3.png"
