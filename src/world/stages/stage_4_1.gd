...
## 4-1 '어둠의 입구' — 4구역 진입 스테이지.
## 22시 깊은 밤, ABILITY(light_leap) 잠금 — 3구역 보스 보상으로 해금.
## 적 3 (zone3 placeholder — Step 7에서 zone4 적으로 교체).
## 그림자 시각: BG/Floor/Platform 매우 어두운 보라톤 + DustOverlay 옅은 결정 입자.
## 아트 리소스가 있으면 적용, 없으면 ColorRect 플레이스홀더 유지.

const STAGE_ID := "stage_4_1"
const SPAWN_POSITION := Vector2(80, 320)
const BG_PATH := "res://assets/backgrounds/bg_stage_4_1.png"
