...
## 2-6 '안개 갈림길' — NONE 잠금, 호흡 + 2-H 숨김 발견 위치.
## 11시(낮), 적 3 (zone2 베이스 — weeping_willow + mire_stone + miasma_lily).
## Lens 회전으로 LightSensor(sensor_2_6_hidden) 활성화 → HiddenRevealer가 stage_2_h 진입 PortalHidden 노출.
## 2-7로 가는 우측 메인 경로 + 2-H로 가는 숨김 경로 분기.

const STAGE_ID := "stage_2_6"
const SPAWN_POSITION := Vector2(80, 320)
const BG_PATH := "res://assets/backgrounds/bg_stage_2_6.png"
