...
### B-6. 일식 링 시간 자원 게이지 (80×80)

| # | 사용처 | 규격 | 폴더 | 파일명 | 비고 |
|---|---|---|---|---|---|
| 114 | 자원 링 베이스 | **80x80** | `assets/ui/hud/` | `hud_time_gauge_ring.png` | 외곽 6px 링(금 `#F2CC66` 또는 그레이스케일). 링 중심 radius 40. Pass 1.5=PNG 드롭인 즉시 반영, Pass 2=shader 아크 마스킹. **규격 재확장 2026-04-23** (40→80, 픽셀 버짓 +300%) |
| 115 | 자원 링 그림자 잠식 레이어 | **80x80** | `assets/ui/hud/` | `hud_time_gauge_shadow.png` | 동일 80×80. 그림자 `#1A1410`. Pass 2 shader 도입 후 활용. 0%=완전 일식 |
| 116 | 자원 링 경고 오버레이 | **80x80** | `assets/ui/hud/` | `hud_time_gauge_warning.png` | 붉은 `#B83A3A` 오버레이. 20% 이하 시 1초 주기 맥동(코드가 modulate 알파 펄스 적용). STOPPED 시 50% dim |
