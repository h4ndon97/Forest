...
**컨셉 (변경)**: 빛/그림자 빔을 물리적으로 막는 **고대 룬 코어 + 부유 파편**. 중앙에 룬이 새겨진 석판 코어, 주변에 쪼개진 돌 파편 4개가 각자 다른 위상으로 위아래 부유. 거울과 대비되지만 **"정적이 아닌 자기 리듬"**을 가진 유물 느낌.

**색 팔레트**:
- 석판/파편 기본: #665E59 (짙은 회색 따뜻톤)
- 하이라이트 엣지: #857F74
- 그림자 엣지: #3D3833
- 균열 라인: #242120
- 이끼 진한색 (선택 포인트): #335730
- 이끼 밝은색: #5C8548

**룬 발광색 (형광 하늘색)**:
- 룬 코어 (1~2px 가장 밝은 심): **#E8FCFF** (거의 흰색)
- 룬 선 메인: **#7DDFFF**
- 내측 바림 (1px): **#3AA8D6**
- 외측 바림 (1px): **#1E5A78**
- 그리는 순서: 외측 → 내측 → 메인 → 코어 (안쪽일수록 밝게)

**파츠별 상세**:
- **룬 코어 (24x32)**: 세로 직사각 작은 석판. 중앙에 룬 문양 1개. 엣지에 균열 1~2개, 하단에 이끼 소량 (선택). 좌우 대칭
- **파편 1~4**: 크기는 제각각(균일하지 않게)이지만 대략 16x16 / 12x12 규격 내. 모서리가 깨진 비정형 다각형. 파편별 실루엣 차별화. 이끼는 1~2개 파편에만 포인트로
- **단판 전제**: 움직임(부유, 미세 회전, 룬 발광 맥동)은 전부 **코드에서 처리**. 사용자는 정적 단판만 제작
...
|---|---|---|---|---|
| 룬 | ±1.0px | 3.6 | 0 | 없음 |
| 파편 1 | ±2.5px | 2.2 | 0 | ±0.03 rad |
| 파편 2 | ±2.5px | 2.4 | π | ±0.04 rad |
| 파편 3 | ±1.8px | 2.8 | π/3 | ±0.025 rad |
| 파편 4 | ±2.0px | 2.6 | 1.4π | ±0.03 rad |

**룬 발광 (코드)**:
- 동일 `cover_rune.png`를 Sprite2D 1장 더 깔고 `CanvasItemMaterial.BLEND_MODE_ADD` + 하늘색 modulate
- 스케일 1.18x, 알파 0.05 ↔ 0.55 맥동 (주기 2.8초) — 숨쉬는 듯한 오라

**투영 영역 (65, 66)**: 본체 기준 앞쪽(빛 반대 방향) 96×32 직사각형. 회전은 Godot에서 `shadow_direction` 기반으로 적용되므로 스프라이트는 0° 기준 1장

**Fallback 현황 (Phase 2-5b 완료)**:
- 스프라이트 부재 시: `cover_visual.gd` 의 `_draw()` 가 단일 석판 교보재 렌더 (회색 석판 + 균열 + 이끼)
- `cover_rune.png` 로드되면 자동으로 룬+파편 모드 전환 (코드는 자동 준비됨)
- 파편 스프라이트 없으면 해당 자리만 비워짐 → 단계적 추가 가능
- 투영 영역: `ProjectionVisual` ColorRect 96×32, 색 #0C0C19 alpha 0.35
- 하이라이트: `environment_highlight.gd` Line2D 40×56 노란 테두리 (거울과 공용)
- 프롬프트: `environment_prompt.gd` Label "[E] 차폐물 밀기"

**우선순위**: 64a(룬 코어) > 64b/c/d/e(파편 4) > 65(정적 투영) > 66(흐름 중 투영)
...
| # | 사용처 | 규격 | 폴더 | 파일명 | 비고 |
|---|---|---|---|---|---|
| 111 | 궤도 시계 외곽 링 | **64x64** | `assets/ui/hud/` | `hud_time_clock_ring.png` | 금속질 다크브라운 `#1A1410` 4px 링. 상태별 modulate 색(금/시안/금↔시안 보간)으로 재사용. STOPPED 시 1초 주기 미세 펄스. **규격 재확장 2026-04-23** (핵심 기믹 UI 비중 상향, 32→48→64, 원본 대비 픽셀 버짓 +300%) |
| 112 | 해 (태양) 발광 루프 | **32x32 단판 또는 128x32 (4프레임)** | `assets/ui/hud/` | `hud_time_sun.png` | 궤도 내부에서 회전 위치 이동. 금색 `#F2CC66` 발광. 캔버스는 발광/글로우 여백 포함 (내부 코어 16~20px). 1프레임 단판 제공 시 코드가 알파 호흡(0.7~1.0) 자동 보완 |
| 113 | 달 (초승달) 발광 루프 | **32x32 단판 또는 128x32 (4프레임)** | `assets/ui/hud/` | `hud_time_moon.png` | 해와 180° 대립 위치. 시안 `#A8D8D0` 초승달. 발광/글로우 여백 포함. 단판 시 알파 호흡 자동 보완 |

### B-6. 일식 링 시간 자원 게이지 (128×128 팔각형)

| # | 사용처 | 규격 | 폴더 | 파일명 | 비고 |
|---|---|---|---|---|---|
| 114 | 자원 링 베이스 | **128x128 팔각형** | `assets/ui/hud/` | `hud_time_gauge_ring.png` | 정통 팔각형, 꼭짓점 8개 (12시·1:30·3·4:30·6·7:30·9·10:30 위치). 꼭짓점 radius 46, 변 중앙 radius 42, 안쪽 radius 34. 금 `#F2CC66` 또는 그레이스케일. **형태 결정 2026-04-23** (원형→팔각, 팔괘 모티프) |
| 115 | 자원 링 그림자 잠식 레이어 | **128x128 팔각형** | `assets/ui/hud/` | `hud_time_gauge_shadow.png` | 동일 팔각형 형태. 그림자 `#1A1410` + 보라 `#8B2FC6` 경계. Pass 2 shader 도입 후 활용. 0%=완전 일식 |
| 116 | 자원 링 경고 오버레이 | **128x128 팔각형** | `assets/ui/hud/` | `hud_time_gauge_warning.png` | 동일 팔각형 형태. 붉은 `#B83A3A` 반투명 오버레이. 20% 이하 시 1초 주기 맥동(코드가 modulate 알파 펄스 적용). STOPPED 시 50% dim |
...
> **구현 상태 (2026-04-24)**: Pass 5 Step 1 ①③ 드롭인 인프라 완료. 아트 없이도 동작.

| # | 사용처 | 규격 | 폴더 | 파일명 | 비고 |
|---|---|---|---|---|---|
| 50 | 체력 크랙 오버레이 | 640x360 | `assets/ui/` | `hud_hp_crack.png` | HP 20% 이하에서 화면 전체에 실금 오버레이. 알파 채널로 실금만 보이게. **드롭인 완료 (Pass 5 Step 1 ①)** — `OverlayHpCrack`이 경로 로드 시도, 없으면 visible=false 유지. 에셋 추가 시 게임 재시작으로 즉시 반영 |
| 51 | 디졸브 노이즈 텍스처 | 256x256, 타일링 | `assets/ui/` | `overlay_dissolve_noise.png` | 메뉴 전환용. 그레이스케일. shader threshold로 디졸브 효과. **드롭인 완료 (Pass 5 Step 1 ③)** — shader 내 hash value_noise로 현재 동작 중, 에셋 추가 시 `dissolve_transition.gdshader`에 `noise_texture` uniform 추가해 교체 |

### 제작 가이드

**크랙 오버레이**: 검은색 얇은 실금. 화면 가장자리에서 중앙으로 퍼지는 방사형 패턴. 투명도는 HP 비율에 따라 코드에서 조절. `OverlayHpCrack.modulate.a`가 0~0.85 범위로 구동되므로 PNG 자체는 불투명 실금 패턴만 그려두면 됨.

**디졸브 노이즈**: 부드러운 펄린 노이즈 또는 셀룰러 노이즈. 타일링 가능해야 함 (상하좌우 이음새 없음). Aseprite 기본 기능 또는 외부 툴로 생성. 현재 shader 내 `value_noise(UV * 32.0)`로 동작 — PNG 교체 시 스케일(`noise_scale` uniform) 기본값 32 기준으로 그레이스케일 이미지를 256×256으로.

**Fallback**: 두 파일 모두 없어도 ① 크랙 = no-op(알파 0 유지), ③ 디졸브 = shader hash noise로 동작.

**우선순위**: 50(체감 큼) > 51(메뉴 전환, shader fallback 품질 이미 충분)
...
> **구현 상태 (2026-04-24)**: Pass 5 Step 2 드롭인 완료. D7-5 확정으로 "낮/밤 2종"으로 압축. 4×4 단색 fallback 동작 중.

| # | 사용처 | 규격 | 폴더 | 파일명 | 비고 |
|---|---|---|---|---|---|
| 52 | 앰비언트 파티클 (낮) | 4x4 ~ 8x8 | `assets/sprites/effects/` | `particle_ambient_day.png` | **흰 꽃가루** (D7-5 확정). `AmbientParticles` 씬 DAY_TEXTURE_PATH에서 로드. 드롭인 완료. 내부 채색(흰~연황)까지 아트에 담거나, 흰/회색 단계만 두고 코드 color_ramp가 부여하는 방식 모두 허용 |
| 53 | 앰비언트 파티클 (밤) | 4x4 ~ 8x8 | `assets/sprites/effects/` | `particle_ambient_night.png` | **녹색 반딧불** (D7-5 확정). NIGHT_TEXTURE_PATH에서 로드. 작은 원형 발광 모양 권장. 내부 채색(녹색)까지 아트에 담으면 됨 — 현재 코드는 `Color(0.4, 1.0, 0.4, 0.9)` fallback만 사용 |

### 제작 가이드

**D7-5 확정 사양**:
- 낮(DAY/DUSK): 흰 꽃가루 `Color(0.95, 0.95, 0.9, 0.7)`, 8px 불규칙 얼룩 권장
- 밤(NIGHT): 녹색 반딧불 `Color(0.4, 1.0, 0.4, 0.9)`, 작은 원형 발광 권장

**구현된 파티클 파라미터** (`src/world/ambient/ambient_particles.gd` 상수 — 초기값 보수적 50%):
- 낮: amount 8, lifetime 8s, velocity 4~10, gravity `(-3, 3, 0)` (바람 + 약한 중력)
- 밤: amount 6, lifetime 6s, velocity 2~6, gravity `(0, 0, 0)` (부유)

**배치**: `Player.tscn`의 Camera2D 자식으로 배치됨 (카메라 이동 따라옴). EMISSION_BOX 320×180.

**전환**: `EventBus.day_night_changed` 구독 → 1.0s 병렬 Tween(페이드 인 + 페이드 아웃 → emitting=false). 시간 정지 시 `freezable_particles` 그룹을 통해 자동 정지.

**우선순위**: 52 ≈ 53 (낮/밤 둘 다 동등 비중 — 게임 시간대 전환 자주 발생)
...
| Pass 5a | 50~51 | 2 | UI 감성 (크랙, 디졸브) — **드롭인 완료 (2026-04-24)** |
| Pass 5b | 52~53 | 2 | 환경 분위기 (앰비언트 파티클, 낮/밤 2종) — **드롭인 완료 (2026-04-24)** |
| Pass 5c | 54~56 | 3 | 슬래시 트레일 (기본/피니시/속성별) — Step 3 예정 |
...
**선제 결정 현황** (`docs/EFFECTS.md` §5 D7 참조):
- ✅ 선택지 #1 (시간 정지 톤): 세피아 화면 전체 균일 — #49 폐기 or 재정의 필요
- ✅ 선택지 #2 (힛 플래시 색): 피니시 속성별 분기 → #55, #56 속성별 제작 (Pass 5 Step 3 진입 시)
- ✅ 선택지 #3 (땅거미 경고 색): 거리별 보라→빨강 보간 — #43 색상 확정
- ✅ 선택지 #4 (데미지 넘버 스타일): Galmuri11 비트맵 — #47 폐기
- ✅ 선택지 #5 (앰비언트 파티클 종류): 낮=흰 꽃가루 / 밤=녹색 반딧불 — #52/#53 파일명·규격 확정 및 드롭인 완료
