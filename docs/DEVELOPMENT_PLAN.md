...
#### 3-7. 1구역 아트 + 이펙트 적용
- [x] **이펙트 Pass 1 프레임워크** (2026-04-19) — EffectsSystem/OverlaySystem Autoload + 카메라 쉐이크(trauma²) + 힛플래시 셰이더 + 힛스톱 + effects_config.tres + 디버그 키 F6~F9 + Damageable 3건(플레이어/적/보스) 연동
- [ ] 플레이어 스프라이트 + 애니메이션
- [ ] 1구역 타일셋 (빛의 숲)
- [ ] 1구역 적/보스 스프라이트
- [ ] 1구역 배경
...
## 구현 현황 요약 (최종 업데이트: 2026-04-19, Phase 3-7 Pass 3 Step 3 완료)
...
| **3-7 Pass 3 Step 1** | **시간 정지 세피아 프레임워크** | **✅ 완료 (25acfa7)** | **`assets/shaders/effects/time_stop_sepia.gdshader` 신규(canvas_item, `hint_screen_texture` + `mix(original, sepia, weight)` 화면 전체 균일) + `EffectsTimeStop` RefCounted 헬퍼(~50줄, `apply_instant(on)` + `_install_shader()`) + `EffectsConfigData` Time Stop 그룹 9필드(sepia_tone/tint_color/saturation_lerp/weight_target/transition_duration + blue_pulse/afterimage 예약) + EffectsSystem 통합(`_time_stop` 멤버 + `debug_toggle_time_stop()`) + 디버그 키 F12(F10/F11 InventorySystem 선점 회피). **D7-1 재조정**: 원안 "주변부 색상 유지"는 사용자 체감 검증 결과 "터널 시야"처럼 보여 폐기 → 화면 전체 균일 세피아. edge_mask uniform 제거. EFFECTS.md §5-1 동시 갱신. EventBus 구독/Tween/파티클/펄스/잔상은 Step 2~4 별도. 신규 class_name `EffectsTimeStop` 등록 시 `--editor --headless --quit` 1회 필요** |
| **3-7 Pass 3 Step 2** | **시간 정지 Tween 트랜지션** | **✅ 완료 (904bace)** | **`EffectsTimeStop`이 `EventBus.time_flow_stopped/started` 구독 → 신규 `apply_transition(on)`으로 weight 0↔`time_stop_weight_target` Tween(`time_stop_transition_duration`=0.30s, `create_tween().set_ignore_time_scale(true).tween_method(...)`). `_weight_tween.kill()` 재진입 안전(현재값→목표값 점프 없이 이어 감). `_current_weight` 로컬 추적(OverlaySystem getter 부재). `apply_instant`은 초기화 경로로 유지(Tween도 kill). `EffectsSystem.debug_toggle_time_stop`은 `apply_transition` 호출로 전환 → F12가 실제 트랜지션 미리보기로 동작(TimeSystem 상태 비오염, 적 정지/클럭 사이드이펙트 회피). 수정 파일 2(effects_time_stop.gd +~35줄, effects_system.gd 1줄). **검증**: gdlint 클린, 헤드리스 로드 클린** |
| **3-5 재설계 Step 1** | **월드맵 동심 극좌표 — 데이터 스키마 + zone_1 위치 패치** | **⏳ 진행 중 (2026-04-20)** | **WORLD_MAP_REDESIGN.md 확정 (8 결정). StageData +3필드(radius_ring/angle_deg/radius_offset) + zone_1 10개 .tres 패치(SV=180°/BC=270°, 6시 시작 CCW 270° 아크, 1-h radius_offset=-20). Phase 3-7 Pass 3 와 병행(파일/시그널 충돌 없음). Step 2~4 별도 승인 후 진행** |
| **3-7 Pass 3 Step 3** | **Freezable 그룹 + 플레이어 숨결** | **✅ 완료 (4357e1c)** | **`EffectsFreezable` RefCounted 헬퍼 신규(~35줄) — `EventBus.time_flow_stopped/started` 구독 → `_host.get_tree().call_group(&"freezable_particles", &"set", &"speed_scale", 0.0 or 1.0)` + `apply(frozen: bool)` 수동 토글. `EffectsSystem._freezable` 멤버 + `_ready` 인스턴스화 + `debug_toggle_time_stop`이 셰이더 Tween·그룹 토글 동시 호출(TimeSystem 비오염 유지). **Player.tscn** 신규 노드 `BreathParticles` GPUParticles2D — 위치 `(4,-28)`, amount=2 / lifetime=1.8s / explosiveness=0 / randomness=0.6 / speed_scale=1.0, 사용자 체감 튜닝 후 확정값(초안 대비 속도·수량 ~50% 하향). process_material + GradientTexture1D/2D + Gradient 5개 모두 scene sub_resource로 베이킹. **`freezable_particles` 그룹 비가입**(시간 정지 예외). 설계 결정: 단일 책임 분리(EffectsTimeStop=셰이더, EffectsFreezable=그룹) / `EffectsParticlePresetsData`에 breath_* 필드 미도입(피격 카테고리 의미 집중 유지) / player.gd 수정 0줄(scene 베이킹만, 커플링 없음). 현재 그룹 멤버 0개 — Pass 5 앰비언트 파티클 합류 시 자동 편입. **검증**: gdlint 클린(2개 파일), `--editor --headless --quit` 캐시 갱신 후 `--headless --quit` 로드 클린** |
