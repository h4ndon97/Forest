...
### 베이스 4종의 1구역 서브 타입 (가안)

| 베이스 | 1구역 서브 타입 | 외형 방향 | 행동 차별화 |
|---|---|---|---|
| 나무 | **어린 참나무** | 작고 동글한 나무 괴물 | melee 범위 축소 (도입용), 느림 유지 |
| 바위 | **이끼 바위** | 이끼 덮인 둥근 바위 | Defense 감산만 유지, 경직 저항 완화 |
| 꽃 | **빛 꽃** | 빛이 새어나오는 꽃잎 | 분열체 2 → 3마리, HP 더 낮게 |
| 돌기둥 | **이정표 돌** | 낮고 가는 돌기둥 | ranged 유지, 투사체 속도 ↓ (도입 난이도) |

### 1구역 고유 적 (3안 중 택 1)

| 안 | 이름 | 컨셉 |
|---|---|---|
| A | **빛나비 (Luminous Moth)** | 작고 빠른 공중 적, 낮에만 출현, 밤에는 등불 주변으로 모여듦 |
| B | **이끼 정령 (Moss Sprite)** | 지면을 기어다니는 작은 적, HP 낮고 다수 스폰, 플레이어 시야 방해 |
| C | **빛가루 포자 (Pollen Spore)** | 꽃 사망 시 추가 스폰되는 공중 포자, 방향 유도 이동 |

### 결정 대기
- **D3** 1구역 고유 적 3안 중 1 — "몽환·빛이 쏟아짐" 테마 정합 기준 사용자 결정
