...
| 2026-04-19 | v1.6: **일시정지 메뉴 구현 완료**. 3-메뉴(이어하기/설정/타이틀로) + "※ 마지막 거점에서 이어집니다" 보조 라벨. `pause` 액션 신규 등록(ESC), autoload `PauseMenu` CanvasLayer(layer=95) + `PROCESS_MODE_ALWAYS`. 진입 시 `get_tree().paused=true` + BGM Master -12dB + 알파 60% 어두운 베일. 차단 조건: 인게임 아님/전환 중/대화 중/Inventory·WorldMap·Shop 열림. EventBus 3건 신설 — `game_paused`/`game_resumed`/`return_to_title_requested`. 설정 패널은 `title_settings_panel` preload 재사용. StageSystem이 `return_to_title_requested` 수신 → `change_scene_to_file.call_deferred(TitleScreen)`. 레퍼런스 리서치 기반 결정(HK/Blasphemous/Salt and Sanctuary): 경고 팝업 없이 마지막 거점 복귀, 설정 패널 공유. |
| 2026-04-19 | v1.7: **Phase 3-6 메뉴 마감 완료 (3-6-a/b/c)**. (a) 월드맵 상세 패널 — `world_map_detail_panel` PanelContainer 우측 (480, 60)/(140, 240), 이름·구역·시각·클리어·잠금(빛/정화/환경/능력)·⚠·인접 실시간 갱신, B안 단일 Enter 이동. (b) 인벤토리 5-파일 분리 — `inventory_menu`(orchestrator) + `InventoryTabController`(Q/E 탭) + `EquipmentTab`(기존 로직 이관) + `SkillTab`(4슬롯+해금 목록 2-컬럼) + `SkillTabNavigator`(RefCounted 컬럼 상태). 키 라우팅: Q/E 탭 / W·S·↑·↓ 항목 / A·D·←·→ 컬럼(스킬 탭) / F·Enter 양 탭 공통 장착 / J 탭별 분기(장비=장착, 스킬=해제). **`attack` 액션 라우팅 제거**로 KEY_J 물리키 단일 경로 — 이중 발화 차단. 거점 외부에선 변경 액션 무시 + 힌트 라벨로 안내. `SkillSystem._auto_equip_debug` → `DEBUG_SKILL_AUTO_EQUIP` 상수 게이팅. (c) 공통 컴포넌트 — `MenuFrame`(베일+패널+제목+힌트+content_root) + `MenuSelectionRect`(호흡 1.0s ±0.10) class_name 추출. `pause_menu`/`pause_menu_controller`가 마이그레이션(224→155, 180→145), `title_menu_controller`는 SelectionRect만 공통화(전체 frame은 4-CanvasLayer 구조상 보류). KEYMAP v0.3 메뉴 컨텍스트 표 추가. 미니맵은 Phase 4 이월. 신규 파일 7개 모두 < 300줄, gdlint/gdformat 통과, headless 로드 클린. 상세=`PHASE_3_PLAN.md` §8 구현 결과 |
