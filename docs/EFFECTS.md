...
## 5. 아트 디렉션 선택지 (구현 전 결정 필요)

> 사용자가 선택해야 구현이 진행된다. 기본 추천은 굵게.

1. **시간 정지 화면 톤**
   - (A) 블루 틴트 + 채도 70% 감소 (Katana Zero)
   - **(B) 청백색 단색 + 하이라이트 (시간이 얼어붙은 느낌)** ← 추천
   - (C) 세피아 + 주변부만 색상 유지

2. **힛 플래시 색상**
   - (A) 흰색 (Hollow Knight 표준)
   - **(B) 피니시 속성별 (빛=흰, 어둠=보라, 불=주황)** ← 추천 (피니시 시스템과 연동)
...
4. **데미지 넘버 스타일**
   - (A) 비트맵 픽셀 폰트 (Hollow Knight 톤)
   - (B) Label + 아웃라인 셰이더 (Dead Cells 톤)

5. **앰비언트 파티클 종류**
   - 숲 기본: 녹색 반딧불? 흰 꽃가루? 갈색 낙엽?
   - 밤 시간대 전용 파티클?

6. **HUD 체력 표시 형태**
   - (A) 가로 바 (표준)
   - (B) 원형 미터 (Hollow Knight Soul 스타일)
   - (C) 구슬/상처 아이콘 (Blasphemous 스타일)
