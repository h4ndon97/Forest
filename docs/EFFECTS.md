...
#### Pass 5. UI/환경 폴리시 (Step 1~7 상세 계획 — 2026-04-24)

> **Step 분할 근거**: Pass 1~4 인프라 재사용 가능 단계(Step 1~4) → 1구역 아트/결정 의존 단계(Step 5~7) 순서.
>
> **범위 조정 (2026-04-24)**:
> - **타이프라이터 대화창**: 대화 시스템 자체가 미확정이므로 Pass 5 범위 밖. 대화 시스템 설계 착수 시 별도 진행.
> - **보스 페이즈별 외형 변경(⑭)**: 범위 팽창 우려로 Phase 4 본격 콘텐츠로 이월 (memory: `project_boss_ui_decisions`).
> - **INARI HDR 글로우 / 방사형 burst**: Pass 3 Step 4에서 이월된 레퍼런스를 Step 6(글로우)·Step 4(burst)로 분산 통합.

- **Step 0** ✅ 흐름 통일 (2026-04-25) — 시스템 간 트랜지션 동기. 코드/데이터 변경만, 아트 의존 없음
  - **OverlaySystem 표준 timing 상수**: `TIMING_QUICK=0.18` / `TIMING_SOFT=0.30` / `HOLD_SHORT=1.2` / `HOLD_LONG=1.5` / `HUD_FADE_IN_DELAY=0.4` / `HUD_FADE_IN_DURATION=0.6`. 메뉴 디졸브·아이템 팝업이 같은 호흡 사용
  - **`stage_transition_requested` 시그너처 확장**: 3번째 인자 `options: Dictionary` — `fade_color` / `skip_fade_out` / `fade_in_duration`. 기존 emit 7곳 모두 빈 `{}` 추가
  - **`stage_transition.execute(scene_path, direction, options)`**: 색상 인자화 + skip_fade_out 분기 (RGB만 갱신, alpha tween)
  - **죽음 시퀀스 분위기형 통합**: `combat_config.tres`에 `respawn_fade_in_duration=0.6` / `respawn_hold_duration=1.5` / `respawn_fade_out_duration=0.4` 외부화. `combat_hud._death_overlay` 알파 0→1.0 풀 페이드(이전 0.6 정지) → 1.5s 홀드 → `combat_system._on_respawn_timeout`이 `skip_fade_out=true`로 stage_transition 호출 → 단일 페이드인 0.4s. 이전 "검정-회색-검정" 다중 단계 → "검정 → 거점" 3박자
  - **HUD 페이드인**: `combat_hud._hud_root` (MarginContainer) modulate.a 0 시작, 첫 `stage_entered` 시 0.4s 대기 후 0.6s 페이드인. `_death_overlay`는 별도 노드라 죽음 시퀀스 충돌 없음
  - **인터페이스만 준비, 실 적용 미뤄둔 것**: `fade_color` 호출부는 모두 default(검정) — TimeSystem 시간대 결정 후 낮/밤 모티프 분기 적용 (B2)
- **Step 1** 📋 UI 감성 (5a) — Pass 1~2 인프라로 아트 fallback 동작 가능
  - ① **빈사 크랙 오버레이**: `OverlaySystem.set_hp_crack_ratio(ratio)` API 신규. HP<20% 구간에서 `ratio = 1 - hp/0.20` (clamp 0~1). EventBus `player_health_changed` 구독. 에셋 #50 `hud_hp_crack.png` fallback 허용(없으면 코드 그라디언트)
  - ② **딜레이드 HP pip**: 뒤 pip가 0.3s 지연 꺼짐 Tween. 기존 `player_hud` 렌더러 확장
  - ③ **메뉴 디졸브**: `assets/shaders/effects/dissolve_transition.gdshader` (canvas_item, noise threshold) + `OverlaySystem.play_dissolve(duration, direction)` API. 에셋 #51 `overlay_dissolve_noise.png` fallback=shader 내 hash noise. 인벤토리·메뉴 진입·퇴장 트리거
  - ④ **아이템 획득 팝업**: 신규 `src/ui/hud/item_pickup_popup.tscn` (Label + 16×16 아이콘, slide-in 0.2s + hold 1.5s + fade 0.3s). `InventorySystem.item_picked_up(item_id, count)` 시그널 구독
- **Step 2** 📋 앰비언트 파티클 (5b-⑥) — D7-5 적용, Pass 3 freezable 인프라 자동 편입
  - **낮(DAY/DUSK)**: 흰 꽃가루. color (0.95, 0.95, 0.9, 0.7), lifetime 8s, gravity 약, 수평 바람 벡터
  - **밤(NIGHT)**: 녹색 반딧불 + PointLight2D 자식. color (0.4, 1.0, 0.4, 0.9), lifetime 6s, sin 오프셋 부유
  - 신규 씬 `src/entities/world/ambient_particles.tscn` — 카메라 자식 배치 대상. `EventBus.time_of_day_changed` 구독 → emitting 토글 + alpha Tween 1.0s
  - **`freezable_particles` 그룹 가입** → 시간 정지 시 `speed_scale=0` 자동 적용 (Pass 3 Step 3 인프라)
  - 에셋 #52 / #53 fallback 4×4 흰 단색 허용. 초기값은 수량·spread·속도 ~50%로 보수적 시작 (memory: `feedback_particle_initial_values`)
- **Step 3** 📋 슬래시 트레일 (5c-⑧⑨) — Pass 2 피니시 속성 체인 재사용
  - **기본 트레일** (콤보 1~3타): Line2D 점 shift + 0.15s 페이드 / 또는 #54 `slash_basic.png` 5프레임 단발
  - **피니시 트레일** (콤보 4타): #55 / #56 `slash_finish_<attr>.png` 속성별 단발
  - `EffectsSystem.request_slash_trail(origin, dir, is_finish, attribute)` 신규 API. 색 매핑은 `get_finish_color(attr)` 재사용 (light=흰 / shadow=보라 / hybrid=금)
  - 헬퍼 `EffectsSlashTrail` RefCounted, Sprite 풀 8장
- **Step 4** 📋 피니시 컷인 + INARI 방사형 burst (5c-⑩·⑫) — 0.6s 시퀀스
  - 트리거: 보스/엘리트 HP 0 직전 피니시 일격 (`base_boss._on_finish_blow`)
  - 시퀀스: (1) Camera zoom 1.0→1.4, 0.2s EASE_OUT (2) `Engine.time_scale=0.2`, 0.4s, ignore_time_scale 경로 (3) CanvasLayer 블랙바 2줄 상·하 16px slide-in/out (4) **방사형 burst** — 피니시 속성색 파티클 8~12방사, 코어+할로 2레이어 복제 (5) 1프레임 white flash (6) 복원
  - 헬퍼 `src/systems/effects/effects_finish_cutin.gd` RefCounted
  - 데이터 외부화: `effects_config.tres` 신규 필드 — `finish_cutin_zoom / slowmo_scale / slowmo_duration / bar_thickness / burst_count`
  - (선택) 셰이더 `assets/shaders/effects/radial_burst.gdshader`
- **Step 5** 📋 보스 연출 폴리시 (⑬) — HP 바 최종 위치 결정
  - 현재 상단 임시 배치(BAR_TOP_MARGIN=16) → 하단 복귀 / 상단 유지 재결정. 미니맵 도입 시 레이아웃 충돌 검토 (ui_design_master F 카테고리와 연동)
  - `PHASE_COLORS` 1~5구역 보스별 팔레트 다양화 검토
  - 페이즈 전환 시 Step 4 방사형 burst 파티클 재사용 (페이즈별 색)
  - ⑭ 페이즈별 외형 변경은 본 Step 범위 밖 — Phase 4로 이월 명시
- **Step 6** 📋 INARI HDR 글로우 베이스라인 (⑪) — 1구역 아트 확정 후 진입
  - WorldEnvironment glow 기본값(intensity / threshold / bloom) 설정
  - **픽셀아트 Nearest 필터 충돌 검증 우선** — 밴딩/뭉개짐 여부 체크
  - 코어/할로 2레이어 규약(§6.7) 적용 대상 확정: 플레이어 · 주요 적 · 보스 · 광원 오브젝트
  - 640×360 베이스 렌더 타겟의 bloom GPU 비용 체크
- **Step 7** 📋 패럴랙스 5층 템플릿 (⑦) — 1구역 배경 아트 의존
  - `ParallaxBackground` + `ParallaxLayer` × 5 템플릿
  - motion_scale 권장 분배: L1 원경(실루엣) 0.1 / L2 중원(나무) 0.3 / L3 중경(덤불) 0.6 / L4 플레이어 플레인 1.0 / L5 전경(풀잎/낙엽) 1.2
  - 템플릿 씬: `src/world/templates/parallax_forest_template.tscn`

**Step 간 의존성 요약**:
- Step 1~2: Pass 1~3 인프라만 있으면 착수 가능, 아트 fallback 허용
- Step 3~4: Pass 2 피니시 속성 체인 필수
- Step 5: 1차 테스트 플레이 후 사용자 피드백 필요
- Step 6~7: 1구역 아트 에셋 확정 의존
...
| 2026-04-25 | **Pass 5 Step 0 (흐름 통일) 완료** — OverlaySystem 표준 timing 상수(TIMING_QUICK/SOFT, HOLD_SHORT/LONG, HUD_FADE_IN_*) 신설. `stage_transition_requested` 시그너처 확장 (options Dict — fade_color/skip_fade_out/fade_in_duration). `stage_transition.execute` 색상 인자화 + skip_fade_out 분기. 죽음 시퀀스 분위기형 통합: combat_config에 respawn_fade_in/hold/fade_out_duration 3개 외부화, combat_hud 검정 풀 페이드(0→1.0) + combat_system이 skip_fade_out=true로 stage_transition 호출하여 "검정-회색-검정" 다중 단계 → "검정 → 거점" 3박자. combat_hud HUD 페이드인 (MarginContainer modulate, 첫 stage_entered 시 0.4s 지연 + 0.6s 페이드). emit 호출부 7곳 모두 빈 `{}` 추가. 사용자 호소 "이어지는 게임 같지 않다"의 70% 해소 — 모티프 색상 분기는 인터페이스만 준비 (B2 TimeSystem 후) |
| 2026-04-24 | Pass 5 상세 계획 확정 — 7개 Step으로 분할(§6 Pass 5). Step 1 UI 감성(크랙/딜레이드 pip/디졸브/아이템 팝업) / Step 2 앰비언트 파티클(낮 꽃가루·밤 반딧불) / Step 3 슬래시 트레일(속성별) / Step 4 피니시 컷인 + 방사형 burst / Step 5 보스 연출 폴리시(HP 바 위치 재결정) / Step 6 INARI HDR 글로우 / Step 7 패럴랙스 5층. **범위 조정**: 타이프라이터 대화창은 대화 시스템 미확정으로 Pass 5 제외, 보스 페이즈별 외형 변경(⑭)은 Phase 4 본격 콘텐츠로 이월, INARI HDR 블룸/방사형 burst는 Step 6·Step 4에 각각 통합. **의존성**: Step 1~4는 Pass 1~4 인프라로 아트 fallback 착수 가능, Step 5는 1차 테스트 플레이 피드백, Step 6~7은 1구역 아트 확정 대기 |
