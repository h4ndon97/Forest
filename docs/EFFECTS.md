...
- **Step 2** (예정): EventBus 구독(`time_flow_started/stopped`) + weight 0↔target Tween 트랜지션 (0.3s, `set_ignore_time_scale(true)`)
- **Step 3** (예정): 정지 중 플레이어 주변 파티클 — `freezable_particles` 그룹 speed_scale=0 토글 + 플레이어 자식 `breath_particles` (speed_scale=1 유지)
- **Step 4** (예정): 해제 시 블루 펄스 (`request_screen_flash` 재사용) + 플레이어 잔상 (별도 `EffectsAfterimage` 헬퍼, 대시 재사용 고려)
